...
def covariance_matrix(data):
    ...
    for i in range(N):
        for j in range(N):
            cov_matrix[i][j] = sum(data[i][k] * data[j][k]
                                   for k in range(len(data[0]))) / (len(data[0]) - 1)
...
def eigen_decomposition(matrix):
    ...
    for _ in range(100):  # Iteration limit
        next_vector = [sum(matrix[i][j] * eigenvector[j]
                           for j in range(n)) for i in range(n)]
        norm = math.sqrt(sum(x ** 2 for x in next_vector))
        eigenvector = [x / norm for x in next_vector]
    eigenvalue = sum(matrix[i][i] * eigenvector[i]
                     for i in range(n))  # Approx eigenvalue
...
def pca(data, n_components=1):
    ...
    projected_data = [[sum(row[i] * eigenvector[i]
