...
def generate_and_save_plots(df, plots_dir, target_column):
    ...
    # Target vs useful features
    for col in numerical_cols:
        if col != target_column:
            fig = px.scatter(
                df, x=col, y=target_column, title=f"{target_column} vs {col}"
            )
...
def run_analysis(file_path, output_directory):
    try:
        ...
        # Generate and save plots
        generate_and_save_plots(df_imputed, dirs["plots"], target_column)

        # Save the best model and metrics
        save_best_model(results, os.path.join(dirs["analysis"], "model_report.txt"))

        # Save all results and model details to pickle
        with open(os.path.join(dirs["pkl"], "model_details.pkl"), "wb") as f:
            pickle.dump(results, f)
            print("Model details saved to pickle.")
