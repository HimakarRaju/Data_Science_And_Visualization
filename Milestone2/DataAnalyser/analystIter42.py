...
# Save the analysis log and results to a Word document, with steps, transformations, models, and plot links
...
def save_analysis_log(
    ...
    # Add general analysis steps and reasons
    ...
    doc.add_paragraph(
        "The dataset was first checked for symbols and spaces in column names, "
        "and appropriate replacements were made to ensure compatibility."
    )

    # Data transformation section
    doc.add_heading("Data Transformation", level=1)
    doc.add_paragraph(
        "The data was transformed using various steps including missing value imputation "
        "(using the mean for numerical columns), normalization of numerical values, "
        "and label encoding for categorical variables."
    )
    ...
    doc.add_paragraph(
        f"The {best_model} outperformed other models in terms of evaluation metrics, which include Mean Absolute Error, "
        "Mean Squared Error, R² (for regression), or confusion matrix and classification report (for classification)."
    )

    # Add link to scatter plots and correlation matrix for regression models
    if best_model:
        ...
        # Add scatter plot links for each feature vs target
    ...
    doc.add_heading("Data Transformation Details", level=1)
    doc.add_paragraph("Imputation method used: Mean imputation for numerical columns.")
    doc.add_paragraph(
        "Normalization: Applied to all numerical columns to scale the values."
    )
    doc.add_paragraph(
        "Label Encoding: Applied to categorical columns for conversion into numerical form."
    )
...
def data_analysis_pipeline(dataset_path, output_directory, log_file, pkl_dir):
    ...
    save_analysis_log(
        log_data=analysis_log,  # This should refer to the global analysis log
        output_dir=created_dirs["analysis"],  # Directory to save the analysis log
        models_info=model_results,  # Results from the model evaluation
        best_model=best_model,  # Best model determined from evaluation
        plots_dir=created_dirs["plots"],  # Directory containing the generated plots
        target_column=target_column,  # The target column used in the analysis
    )
...
if __name__ == "__main__":
    data_analysis_pipeline(
        "C:\\Users\\HimakarRaju\\Desktop\\Milestone2\\Datasets2\\ds_salaries.csv",
        "C:\\Users\\HimakarRaju\\Desktop\\Milestone2\\DataReadOuts2",
