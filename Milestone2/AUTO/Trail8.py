...
class Analyser:
        ...
        try:
            ...
            if not self.files:
                ...
                for file in self.files:
                    ...
                    self.vectorize_text()  # Make sure to implement this if necessary
                    self.apply_models()
    ...
    def vectorize_text(self):
        """Vectorize text columns using CountVectorizer if applicable."""
        for col in self.data.select_dtypes(include=["object"]).columns:
            print(f"Vectorizing text column: {col}")
            vectorizer = CountVectorizer()
            transformed_data = vectorizer.fit_transform(self.data[col])
            # You can store or manipulate transformed_data as needed
    ...
    def finalize_analysis(self, file_name):
        ...
        for model_name, metrics in results.items():
            ...
            doc.add_paragraph(
                f"Train Score: {metrics['train']:.4f}, Test Score: {metrics['test']:.4f}, "
                f"R²: {metrics['r2score']:.4f}, MAE: {metrics['absolute']:.4f}, "
                f"MSE: {metrics['squared']:.4f}"
            )
