...
1. **Directory Handling**: The code checks whether the input is a file or a folder. If it's a folder, it processes all
   CSV files inside the folder.
2. **Saving Files to Specific Folders**: All output files (transformation reports, analysis reports, plots, models,
   etc.) are saved into respective folders in the `workingdir/` directory. The folders are named according to their file
   extensions, such as `csv_files`, `plots`, `models`, and `analysis`.
...
def process_files(input_path, target_column, output_dir):
    ...
    if os.path.isdir(input_path):
        ...
        for file_name in os.listdir(input_path):
            if file_name.endswith('.csv'):
                ...
                process_single_file(file_path, target_column, output_dir)
    elif os.path.isfile(input_path) and input_path.endswith('.csv'):
        # Process a single file
        process_single_file(input_path, target_column, output_dir)
    else:
        print("Invalid file or directory path.")


# Function to process a single file
def process_single_file(file_path, target_column, output_dir):
...
if __name__ == "__main__":
    ...
    process_files(input_path, target_column, output_dir)
```

### Key Features

1. **Handling Both Files and Folders**: The function `process_files` checks whether the input path is a folder or a
   single file. It processes all CSV files in a folder or just the single CSV file.
2. **Organized Output**: All generated files are placed in respective folders:
    - `workingdir/csv_files/`: Transformed CSV files.
    - `workingdir/plots/`: Generated plots saved as PDFs.
    - `workingdir/models/`: Saved models in pickle format.
    - `workingdir/analysis/`: Analysis reports and transformation methods in Word format.
3. **Dynamic File Naming**: The generated files are named using the base filename of the input files, making it easier
   to distinguish outputs.

### How to Use

- Set the `input_path` to either a single CSV file or a folder containing CSV files.
- Set the `target_column` to the name of the column you want to predict in your dataset.
- Set the `output_dir` to the working directory where all results will be stored.
